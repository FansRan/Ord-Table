...
const renderCell = (tasks, tableType, taskType) => {    
    if (taskType === "base")
        return tasks.map(task =>
            <td key={`${task.id}-base`}>
                {tableType === "early" && <span style={{ float: 'left' }}>{task.early_date}&nbsp;&nbsp;</span>}
                {tableType === "late" && <span style={{ float: 'left' }}>{task.late_date}&nbsp;&nbsp;</span>}
                {task.id}
            </td>
        )
...
function TaskRow(props) {
    ...
    return (
        <>
            <tr>
                {props.tableType === "input" && <td>{props.taskType === "base" ? "Tâches" : "T.succ"}</td>}
                {renderCell(listTasks, props.tableType, props.taskType)}
                {props.tableType === "early" && <td><span style={{ float: 'left' }}>{finalDate}&nbsp;&nbsp;</span>Fin</td>}
            </tr>
        </>
    );
}
